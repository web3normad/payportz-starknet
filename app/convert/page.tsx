...
const SUPPORTED_STABLECOINS = [
  { symbol: 'USDC', name: 'USD Coin', icon: '💵', color: 'bg-blue-100 text-blue-800' },
  { symbol: 'USDT', name: 'Tether', icon: '💸', color: 'bg-green-100 text-green-800' },
  { symbol: 'DAI', name: 'Dai', icon: '🟠', color: 'bg-orange-100 text-orange-800' },
  { symbol: 'EURC', name: 'Euro Coin', icon: '💶', color: 'bg-yellow-100 text-yellow-800' },
  { symbol: 'CNHC', name: 'CNH Coin', icon: '💴', color: 'bg-red-100 text-red-800' }
];
...
export default function ConvertCurrency() {
  ...
  const address = localStorage.getItem('payportz_chipiwallet_address');
  ...
  const currentFiat = SUPPORTED_FIAT_CURRENCIES.find(fiat => fiat.code === selectedFiat);
  const currentStablecoin = SUPPORTED_STABLECOINS.find(coin => coin.symbol === selectedStablecoin);

  // Calculate conversion data
  const amount = parseFloat(fromAmount) || 0;
  const currentRate = MOCK_EXCHANGE_RATES[selectedFiat as keyof typeof MOCK_EXCHANGE_RATES]?.[selectedStablecoin as keyof typeof MOCK_EXCHANGE_RATES] || 1;
  const rateData = amount > 0 ? {
    rate: currentRate,
  ...
  const handleConfirmConversion = () => {
    ...
    setTimeout(() => {
      ...
      setWalletBalances(prev => ({
        ...prev,
        [selectedStablecoin]: prev[selectedStablecoin as keyof typeof prev] + (rateData?.cryptoAmount || 0)
      }));
                    ...
                    <div className="grid grid-cols-2 gap-4">
                          ...
                          >
                            <div className="flex items-center space-x-3">
                              ...
                              <div className="text-left">
                                <p className="font-medium text-gray-900">{currentFiat?.code}</p>
                                <p className="text-xs text-gray-500">{currentFiat?.name}</p>
                              </div>
                            </div>
                      ...
                      <div className="relative">
                        ...
                        <div className="relative">
                          <button
                            type="button"
                            onClick={() => setShowStablecoinDropdown(!showStablecoinDropdown)}
                            className="w-full flex items-center justify-between p-3 bg-white border border-gray-300 rounded-xl hover:border-gray-400 transition-colors"
                          >
                            <div className="flex items-center space-x-3">
                              <span className="text-2xl">{currentStablecoin?.icon}</span>
                              <div className="text-left">
                                <p className="font-medium text-gray-900">{currentStablecoin?.symbol}</p>
                                <p className="text-xs text-gray-500">{currentStablecoin?.name}</p>
                              </div>
                            </div>
                            <CaretDown size={16} className="text-gray-400" />
                          </button>
                    ...
                    {rateData && fromAmount && (
                      <div className="bg-gray-50 rounded-xl p-4 space-y-3">
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium">1 {selectedStablecoin} = {currentFiat?.symbol}{rateData.rate.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-600">Processing Fee</span>
                          <span className="font-medium">{currentFiat?.symbol}{rateData.fee.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between text-sm pt-3 border-t border-gray-200">
                          <span className="text-gray-800 font-semibold">Total Cost</span>
                          <span className="font-semibold">{currentFiat?.symbol}{rateData.totalAmount.toLocaleString()}</span>
                        </div>
                      </div>
                    )}
            ...
            <div className="space-y-6">
              ...
              {isConnected && (
                <Card className="bg-gradient-to-br from-gray-900 to-gray-800 text-white rounded-2xl border-0">
                  <div className="p-6">
                    <h3 className="font-semibold text-gray-300 mb-4">Your Balances</h3>
                      ...
                      {SUPPORTED_STABLECOINS.map(coin => (
                        <div key={coin.symbol} className="flex items-center justify-between">
                          ...
                          <span className="font-semibold">
                            ${walletBalances[coin.symbol as keyof typeof walletBalances].toFixed(2)}
                          </span>
                        </div>
                      ))}
                    </div>
              ...
              {/* Status Panel */}
              {(applicationStatus === "preview" || applicationStatus === "payment_pending" || applicationStatus === "completed") && rateData && (
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-center space-x-3 mb-6">
                      <div className={`p-2 rounded-full ${
                        applicationStatus === "preview" ? "bg-blue-100 text-blue-600" :
                        applicationStatus === "payment_pending" ? "bg-yellow-100 text-yellow-600" :
                        "bg-green-100 text-green-600"
                      }`}>
                        {applicationStatus === "preview" && <Receipt size={20} />}
                        {applicationStatus === "payment_pending" && <Clock size={20} />}
                        {applicationStatus === "completed" && <CheckCircle size={20} />}
                      </div>
                      <div>
                        <h3 className="font-semibold text-gray-900">
                          {applicationStatus === "preview" && "Order Preview"}
                          {applicationStatus === "payment_pending" && "Awaiting Payment"}
                          {applicationStatus === "completed" && "Conversion Complete"}
                        </h3>
                        <p className="text-sm text-gray-600">
                          {applicationStatus === "preview" && "Review your conversion details"}
                          {applicationStatus === "payment_pending" && "Complete your payment"}
                          {applicationStatus === "completed" && "Funds have been credited"}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div className="bg-gray-50 rounded-xl p-4">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className="font-semibold text-gray-900">{currentFiat?.symbol}{parseFloat(fromAmount).toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-gray-600">You receive</span>
                          <span className="font-semibold text-purple-600">${rateData.cryptoAmount.toFixed(2)} {selectedStablecoin}</span>
                        </div>
                      </div>
                      ...
                      {applicationStatus === "payment_pending" && orderData && (
                        <div className="space-y-4">
                          <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
                            <div className="flex items-start space-x-2">
                              <Warning size={18} className="text-yellow-600 mt-0.5 flex-shrink-0" />
                              <div className="text-sm text-yellow-800">
                                <p className="font-medium">Transfer {currentFiat?.symbol}{rateData.totalAmount.toLocaleString()} to:</p>
                                <p className="mt-1 font-mono">{orderData.paymentDetails.accountNumber}</p>
                                <p className="text-xs mt-2">Reference: {orderData.paymentDetails.reference}</p>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="flex space-x-2">
                            <Button
                              onClick={() => copyToClipboard(orderData.paymentDetails.accountNumber)}
                              variant="outline"
                            ...
                            <Button
                              onClick={() => copyToClipboard(orderData.paymentDetails.reference)}
                              variant="outline"
                              className="flex-1 py-2 rounded-xl"
                            >
                              Copy Reference
                        ...
                        <div className="text-center space-y-4">
                          <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                            ...
                            <p className="text-sm text-green-700 mt-1">
                              ${rateData.cryptoAmount.toFixed(2)} {selectedStablecoin} has been added to your wallet
                            </p>
                          </div>
                          <Button
              ...
              {applicationStatus === "initial" && (
                <Card className="rounded-2xl border-0 bg-white">
                  <div className="p-6">
                    <h3 className="font-semibold text-gray-900 mb-4">Why Convert With Us?</h3>
                    <div className="space-y-3">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="font-medium text-sm">Best Exchange Rates</p>
                          <p className="text-xs text-gray-600">Real-time market rates</p>
                        </div>
                      </div>
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-600">Complete in minutes</p>
                        </div>
                      </div>
                      <div className="flex items-center space-x-3">
